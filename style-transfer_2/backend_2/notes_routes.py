...
def write_note(write_note: NotesModel, Authorize: AuthJWT = Depends()):
    ...
    new_note = Notes(Notes=write_note.Notes, Emotion=write_note.Emotion)
    ...
    response = {"id": new_note.id, "notes": new_note.Notes}
...
@notes_router.get("/emotion/{user}/{date_from}_{to}")
def get_user_emotion_range(user: str, date_from: str, to: str, Authorize: AuthJWT = Depends()):
    try:
        Authorize.jwt_required()
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="invalid Token")
    ...
    admin = session.query(User).filter(User.username == current_user).first()
    target_user = session.query(User).filter(User.username == user).first()

    if admin.is_admin:
        note = (
            session.query(Notes)
            .join(Notes.Users)
            .filter(Notes.username == target_user)
            .filter(Notes.date_created >= date_from, Notes.date_created <= to)
        )
